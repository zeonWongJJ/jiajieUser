  ...
  <div class="periodicReservation">
    <van-popup class="body" v-model="show" position="bottom" :close-on-click-overlay="false">
      ...
      <div class="center">
        ...
        <div class="time">
          <div class="label">
            <div class="li" :class="{checked : checkWeek.unix() == item.moment.unix()}" v-for="(item, index) in week" @click="selectWeek(item.moment)">
              {{index == 0 ? '今天' : index == 1 ? '明天' : item.date}}</div>
          </div>
          <div class="arr">
            <div class="li"
...
<script>
  export default{
    ...
    created() {
      ...
      this.selectWeek(this.checkWeek)
    },
    computed: {
      defaultWeek() {
        return this.checkWeek.day()
      }
    },
    methods:{
      getGray(index) {
      ...
      setArr() {
        ...
        req.service_date = this.checkWeek.year() + '-' + (this.checkWeek.month() + 1) + '-' + this.checkWeek.date()
      ...
      selectTime(time,index){//添加服务时间
        if (time.can_order && !this.getGray(index)) {
          if (JSON.stringify(this.periodicData).indexOf(time.start_at) > -1) {
            this.periodicData = {}
            this.startTime = ''
          } else {
            this.periodicData = {}
