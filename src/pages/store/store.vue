<template>
	<div style="position: relative;">
		<div class="myStore">
          ...
          <div>{{store.store_type == 'SELF_SUPPORT' && user_type != 1 ? '平台净收入（元）' : '累计收入（元）'}}</div>
				...
				<div class="brief_intr_box">
					<span v-if="show_money" class="brief_intr">
            {{statistics.yesterday ? '昨日收入' + statistics.yesterday + '元' : ''}}
            {{store.store_type == 'SELF_SUPPORT' && user_type != 1 ? '' : '，可提现金额'+ statistics.total_income + '元'}}
          </span>
        ...
        <li class="option_li" @click="onachievements()">
          <div class="label">
            <div class="left">
