<template>
	<div class="orderd">
		...
		<div class="body">
			...
			<div class="orderTime" v-if="resOrder.length > 0" style="color: #999;">
        <div>开始时间：{{formatTime(resOrder[0].order_at)}}</div>
        <div>结束时间：{{formatTime(resOrder[resOrder.length - 1].order_at)}}</div>
        <div>预计总服务次数：{{resOrder.length}}</div>
			</div>
      <!--<div class="notime">-->
    ...
    <reservation-week v-model="show" @finish="getOrder" :service_id="store_id"></reservation-week>
	</div>
</template>
...
<script>
  ...
  import reservationWeek from '@/components/reservationWeek'
		export default {
			components:{
			periodicReservation,
			editAddress,
      ...
      formatTime(time) {
        ...
        let data2 = new Date(data.getTime())
        ...
        return year + '-' + month + '-' + day + '(' + this.getDay(time * 1000) + ')'
