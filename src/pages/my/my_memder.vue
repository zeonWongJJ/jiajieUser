<template>
  <div class="member">
    <div class="body" v-if="apiFinish">
      <van-nav-bar :title="show ? '会员中心' : '管理中心'"  />
      <!--tan-->
      <div class="bar_d" :class="{bar_d_2 : show}"
           :style="{height: store_status == 1 && staff_pass == 1 ? '2rem' : '2.42rem'}">
        <ul class="tan_x" v-show="store_status == 1 && staff_pass == 1">
          <li @click="onstore_true()" :class="{tan_x_li:show}">用户版</li>
          <li @click="onstore_show()" :class="{tan_x_li:!show}">{{user_type != 1 ? '商家版' : '清洁师'}}</li>
        </ul>
      ...
      <van-popup v-model="show_ewm_plus">
        <img @click="show_ewm_plus = false" style="width:0.2rem; position: absolute; right:0.2rem; top:0.1rem"
      ...
      <transition name="slide-fade">
        <div class="personal" v-if="show" :style="{top: store_status == 1 && staff_pass == 1 ? '0.84rem' : '0.54rem'}">
          <!--头像-->
          <div class="personal_head_portrait">
            ...
            <ul class="balance">
              <li @click="toBalance">
                <h4>{{(Number(user.user_balance) + Number(user.user_coupon)).toFixed(2)}}</h4>
                <span>余额</span>
              </li>
              <li @click="toCredit">
            ...
            <div class="order">
              ...
              <div class="row">
                ...
                <div class="span" @click="onduiy(3)">
                  ...
                  <div>{{statistics['pending_service'] + statistics['pending_assign']}}</div>
                </div>
    ...
    <loading :show='!apiFinish'></loading>
  </div>
</template>
<script>
