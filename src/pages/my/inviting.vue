    ...
    data() {
      return {
        ...
        share: {
          ...
          , link: `${this.test(window.config.touchURL)}/#/login?user_referee=${this.$route.query.user_id}`
        ...
        , shareLine: `${this.test(window.config.touchURL)}/#/login?user_referee=${this.$route.query.user_id}`
      }
    },
    mounted() {
      ...
      ShareWechat: function (whoToShare) {
        ...
        if (utils.is_weixin()) {
          ...
          if (utils.is_android() || utils.is_ios()) {
            ...
            } else if (utils.is_ios() && window.webkit.messageHandlers.shareToThirdApp) {
