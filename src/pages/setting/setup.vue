...
<script>
	...
	export default {
		...
		methods: { //方法
			...
			loginOut(){
				let that = this;
				that.$fetch('user_logout', {},'','POST',1).then(rs =>{
                    if (utils.is_android() && typeof android != 'undefined' &&  typeof android['loginOut'] != 'undefined') {
                        android.loginOut();
                    }
                    // IOS app
                    if (utils.is_ios() && typeof window['webkit'] != 'undefined' && window.webkit.messageHandlers.loginOut) {
